...
const Orb = ({ active, speaking }: { active: boolean, speaking: boolean }) => {
  return (
    <div className="relative flex items-center justify-center w-32 h-32 md:w-48 md:h-48">
      ...
      {active && (
        <div className={`absolute w-full h-full rounded-full bg-brand-500/20 animate-ping duration-[3000ms]`} style={{ animationDuration: speaking ? '1s' : '3s' }}></div>
      )}
...
export const InterviewRoom = () => {
  ...
  const { isConnected, error, connect, disconnect, sendVideoFrame } = useGeminiLive({
    ...
    onTranscript: (text, isUser) => {
      // Aggregate text into sentences/paragraphs
      setTranscript(prev => {
      ...
      if (!isUser) {
        ...
        aiSpeakingTimeout.current = setTimeout(() => setIsAiSpeaking(false), 2000); // Visual falloff
      }
    }
  });
  ...
  return (
    <div className="h-screen bg-[#f8fafc] flex flex-col overflow-hidden font-sans">
        ...
        <div className="flex items-center gap-4">
           <div className="text-right hidden md:block">
              ...
              <p className="text-xs text-slate-500 flex items-center justify-end gap-1">
                 Senior React Engineer <ShieldCheck className="w-3 h-3 text-brand-600" />
              </p>
           </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* Right Panel: Transcript or Code */}
        <div className={`transition-all duration-300 bg-white border-l border-slate-200 flex flex-col shadow-xl z-10 ${codeMode ? 'w-full md:w-[600px]' : 'w-full md:w-[400px]'}`}>
           ...
           <div className="flex-1 overflow-hidden relative">
              {codeMode ? (
                 <div className="h-full bg-[#1e293b] p-4 overflow-auto font-mono text-sm">
                    <textarea 
                       className="w-full h-full bg-transparent text-slate-300 outline-none resize-none leading-relaxed" 
