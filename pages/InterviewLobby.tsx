...
import { ShieldCheck, Camera, RefreshCw, CheckCircle, ArrowRight, AlertCircle, Users, Loader2 } from 'lucide-react';
...
const Step = ({ active, completed, number, title }: any) => (
    <div className={`flex items-center gap-2 ${active ? 'opacity-100' : 'opacity-40 grayscale'}`}>
        ...
        {number !== 2 && <div className="w-8 md:w-16 h-px bg-slate-200 mx-2"></div>}
    </div>
);
...
export const InterviewLobby = () => {
    ...
    // Overall Process Step (1: Selfie, 2: Join Interview)
    ...
    // Initialize Camera — waits for token loading to finish so video element is in the DOM
    useEffect(() => {
        if (tokenLoading || tokenError) return;

        const shouldShowCamera = step === 1 && !selfieImage;
    ...
    }, [step, selfieImage, tokenLoading, tokenError]);
    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] flex flex-col items-center justify-center p-4 font-sans">
            ...
            {isTokenMode && inviteData && step === 1 && !selfieImage && (
            ...
            <div className="w-full max-w-3xl mb-12 flex justify-center">
                <div className="flex items-center">
                    <Step number={1} title="Selfie Verification" active={step === 1} completed={step > 1} />
                    <Step number={2} title="Join Interview" active={step === 2} completed={step > 2} />
                </div>
            </div>
                        ...
                        <div className="flex justify-center gap-3">
                            {!selfieImage ? (
                                ...
                                <>
                                    ...
                                    <button onClick={() => setStep(2)} className="flex items-center gap-2 bg-brand-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-brand-700 transition-all shadow-lg shadow-brand-500/20">
                        ...
                        {isTokenMode && inviteData ? (
                            <div className="bg-slate-50 rounded-2xl p-6 mb-8 border border-slate-100 text-left max-w-md mx-auto">
                                ...
                                <p className="text-[10px] text-slate-400 flex items-center gap-1.5">
                                    ...
                                    Selfie captured. Identity will be verified during the interview.
                                </p>
                            </div>
                        ) : (
                            <div className="bg-slate-50 rounded-2xl p-6 mb-8 border border-slate-100 text-left max-w-md mx-auto">
