...
function db() { ensureApp(); return (0, firestore_1.getFirestore)(); }
function storage() { ensureApp(); return (0, storage_2.getStorage)(); }
function authAdmin() { ensureApp(); return (0, auth_1.getAuth)(); }
...
const googleServiceAccountKey = (0, params_1.defineSecret)("GOOGLE_SERVICE_ACCOUNT_KEY");
// DocuSign — ISV Connector / JWT Grant secrets
const docusignIntegrationKey = (0, params_1.defineSecret)("DOCUSIGN_INTEGRATION_KEY");
 ...
 */
exports.setOrgClaim = (0, https_1.onCall)({ cors: true, region: "us-central1" }, async (request) => {
...
async function extractTextFromFile(fileBuffer, filePath) {
 ...
 */
exports.screenResume = (0, https_1.onCall)(functionConfig, async (request) => {
    ...
    if (resumeText.startsWith('http') || resumeText.startsWith('gs://')) {
        ...
        try {
            ...
            throw new https_1.HttpsError('internal', 'Could not extract text from the provided resume file.');
        }
    }
    if (!resumeText || resumeText.length < 50) {
        throw new https_1.HttpsError('invalid-argument', 'Resume text is too short or could not be extracted.');
    }
    ...
    try {
        ...
        throw new https_1.HttpsError('internal', error.message || 'Failed to screen resume');
    }
});
/**
    ...
    try {
        ...
        try {
            ...
            if (apiKey) {
                ...
                `.trim();
                resumeEmbedding = await (0, embeddings_1.generateEmbedding)(apiKey, textToEmbed);
                ...
                if (jobData && (!jobData.embedding || jobData.embedding.length === 0)) {
                    ...
                    `.trim();
                    const jobEmbedding = await (0, embeddings_1.generateEmbedding)(apiKey, jobText);
                ...
                if (jobData && jobData.embedding) {
                    const cosine = (0, embeddings_1.calculateCosineSimilarity)(resumeEmbedding, jobData.embedding);
 ...
 */
exports.generateCandidateReport = (0, https_1.onCall)(functionConfig, async (request) => {
    ...
    try {
        ...
        throw new https_1.HttpsError('internal', error.message || 'Failed to generate report');
    }
});
/**
 ...
 */
exports.generateInterviewQuestions = (0, https_1.onCall)(functionConfig, async (request) => {
 ...
 */
exports.analyzeInterview = (0, https_1.onCall)(functionConfig, async (request) => {
 ...
 */
exports.startInterviewSession = (0, https_1.onCall)(functionConfig, async (request) => {
    const { candidate, job, orgId, assessmentId } = request.data;
    if (!candidate || !job) {
        throw new https_1.HttpsError('invalid-argument', 'Missing "candidate" or "job".');
    }
    try {
        const genAI = await getGenAIClient();
 ...
 */
exports.generateJobDescription = (0, https_1.onCall)(functionConfig, async (request) => {
 ...
 */
exports.createGoogleMeetEvent = (0, https_1.onCall)(meetFunctionConfig, async (request) => {
    const { candidateName, candidateEmail, jobTitle, date, // ISO date: "2026-03-01"
    time, // HH:mm: "10:00"
    timezone, // e.g. "Asia/Kolkata"
    durationMinutes = 60, orgId, interviewerEmail // Optional: org user who will host
     } = request.data;
    ...
    try {
        ...
        if (!keyBase64) {
            throw new https_1.HttpsError('failed-precondition', 'Google service account key is not configured. Add GOOGLE_SERVICE_ACCOUNT_KEY secret.');
        }
        ...
        if (error.message?.includes('invalid_grant') || error.message?.includes('JWT')) {
            throw new https_1.HttpsError('failed-precondition', 'Google service account authentication failed. Ensure the service account has Calendar API access and domain-wide delegation is configured.');
        }
        throw new https_1.HttpsError('internal', error.message || 'Failed to create Google Meet event.');
    }
});
// ═══════════════════════════════════════════════════════════════════════════════
...
async function getDocuSignAccessToken() {
    const docusign = await Promise.resolve().then(() => __importStar(require('docusign-esign')));
    ...
    const results = await apiClient.requestJWTUserToken(integrationKey, userId, ['signature', 'impersonation'], Buffer.from(privateKey), 3600 // Token valid for 1 hour
    );
 ...
 */
exports.createDocuSignEnvelope = (0, https_1.onCall)(docusignFunctionConfig, async (request) => {
    const { candidateId, candidateName, candidateEmail, jobTitle, offerContent, // HTML or plain text content of the offer letter
    orgId, companyName, salary, startDate, returnUrl // URL to redirect after signing (optional, for embedded signing)
     } = request.data;
    if (!candidateEmail || !candidateName || !candidateId) {
        throw new https_1.HttpsError('invalid-argument', 'Missing required fields: candidateEmail, candidateName, candidateId.');
    }
    try {
        const accountId = docusignAccountId.value();
        if (!accountId) {
            throw new https_1.HttpsError('failed-precondition', 'DocuSign Account ID not configured. Set DOCUSIGN_ACCOUNT_ID secret.');
        }
        ...
        if (error.message?.includes('consent_required')) {
            throw new https_1.HttpsError('failed-precondition', 'DocuSign consent not granted. The admin must grant consent at: https://account-d.docusign.com/oauth/auth?response_type=code&scope=signature%20impersonation&client_id={INTEGRATION_KEY}&redirect_uri=https://localhost');
        }
        throw new https_1.HttpsError('internal', error.message || 'Failed to create DocuSign envelope.');
    }
});
/**
 ...
 */
exports.checkDocuSignStatus = (0, https_1.onCall)(docusignFunctionConfig, async (request) => {
    const { envelopeId, orgId, candidateId } = request.data;
    if (!envelopeId) {
        throw new https_1.HttpsError('invalid-argument', 'Missing "envelopeId".');
    }
    try {
        const accountId = docusignAccountId.value();
        if (!accountId) {
            throw new https_1.HttpsError('failed-precondition', 'DocuSign Account ID not configured.');
        }
        const { accessToken, basePath } = await getDocuSignAccessToken();
        const docusign = await Promise.resolve().then(() => __importStar(require('docusign-esign')));
        const apiClient = new docusign.ApiClient();
        ...
        throw new https_1.HttpsError('internal', error.message || 'Failed to check DocuSign status.');
    }
});
/**
 ...
 */
exports.docuSignWebhook = (0, https_1.onRequest)({
    cors: true,
    region: "us-central1",
    maxInstances: 10,
    memory: "256MiB",
...
// ═══════════════════════════════════════════════════════════
exports.aiSearchCandidates = (0, https_1.onCall)({ secrets: [geminiApiKey], cors: true, region: "us-central1", memory: "512MiB", timeoutSeconds: 120 }, async (request) => {
    const { query, orgId } = request.data;
    if (!query || typeof query !== 'string' || query.trim().length < 2) {
        throw new https_1.HttpsError('invalid-argument', 'Search query must be at least 2 characters.');
    }
    if (!orgId) {
        throw new https_1.HttpsError('invalid-argument', 'Missing orgId.');
    }
    ...
    const queryEmbedding = await (0, embeddings_1.generateEmbedding)(apiKey, query);
    if (!queryEmbedding || queryEmbedding.length === 0) {
        throw new https_1.HttpsError('internal', 'Failed to generate embedding for search query.');
    }
    ...
    for (const doc of candidatesSnap.docs) {
        ...
        if (!data.embedding || data.embedding.length === 0)
            continue;
        const similarity = (0, embeddings_1.calculateCosineSimilarity)(queryEmbedding, data.embedding);
        scoredCandidates.push({
            ...
            jobId: data.jobId || '',
            experience: (data.experience || []).slice(0, 2).map((e) => `${e.role} at ${e.company}`),
        });
    }
    ...
    try {
        const genAI = await getGenAIClient();
        const candidateContext = topResults.slice(0, 10).map((c, i) => `${i + 1}. ${c.name} — ${c.role} | Skills: ${c.skills.join(', ')} | Location: ${c.location} | Match: ${c.matchScore}% | Summary: ${c.summary?.substring(0, 200) || 'N/A'}`).join('\n');
 ...
 */
exports.scheduledCandidateCleanup = (0, scheduler_1.onSchedule)({
