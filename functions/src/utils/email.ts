...
function getEmailContent(type: EmailType, { name, role, jobTitle, companyName }: Pick<EmailParams, 'name' | 'role' | 'jobTitle' | 'companyName'>): EmailContent {
...
function buildEmailHtml(content: EmailContent, link?: string): string {
    const {
        subject, preheader, icon, iconBg, headline, message,
        detail, buttonText, accentColor, accentLight, footerNote
    } = content;
