...
export const JobApplicationModal: React.FC<JobApplicationModalProps> = ({ job, orgId, onClose, brandingColor }) => {
    const [step, setStep] = useState(1); // 1: Details, 2: Video, 3: Success
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
            <div className="bg-white rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl animate-fade-in-up my-8">
                {/* Header */}
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-8">
                    ...
                    <div className="pt-4 border-t border-slate-100 flex justify-end gap-3">
                        ...
                        <button
                            ...
                            className={`px-8 py-3 font-bold text-white rounded-xl shadow-lg shadow-brand-500/30 transition-all transform active:scale-95 flex items-center gap-2 ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''}`}
                            style={{ backgroundColor: brandingColor }}
                        >
